...
input="/user/ime/fengchaobing/dl_corpus/${datatype}/get_QApairs/lang_divide/${lang}/part-*"
#output="/user/ime/fengchaobing/dl_corpus/${datatype}/ugc_classify/${lang}/"
output="/user/ime/fengchaobing/dl_corpus/${datatype}/get_QApairs/ugc_classify/${lang}"

$hadoopcmd --config "$hadoopconf" fs -rmr ${output}
$hadoopcmd --config "$hadoopconf" jar "$hstream" \
    -D mapred.job.name="get_QApairs - ugc_classify, ${datatype}, ${lang}, fengchaobing" \

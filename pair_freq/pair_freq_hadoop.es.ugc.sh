...
$hadoopcmd --config "$hadoopconf" jar "$hstream" \
    -D mapred.job.name="pair_freq_hadoop.es.ugc, ${datatype}, ${lang}, ${begin}, fengchaobing" \
    ...
    -file "filter_map.py" \
    -file ${pairtxt} \
    -mapper "python filter_map.py ${pairtxt}  $i 2 $1" \
    -reducer "cat" \
    -input ${input1} \
    -input ${input2} \
    -input ${input3} \
...
c4=`awk -F'\t' 'NR=="'"$[$i+1]"'"{print $4}' ${pairtxt}`
c5_=`awk -F'\t' 'NR=="'"$[$i+1]"'"{print $5}' ${pairtxt}`
c5=${c5_:0:-1}
...
if [[ ${#c5} == "0" ]]
then
#    echo "continue----- -----5" >> ${pairfreqfile}
    continue
fi

echo $i, ${c3}, ${c5}
echo -e "#${c3},\t${c5}" >> ${pairfreqfile}
